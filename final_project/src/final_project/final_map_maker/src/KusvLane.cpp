...
bool SKusvLanes::ImportKusvLaneCsvFile(std::string strImportKusvLaneCsvFilePath)
{
	DisplayInfo("Import Name is " + strImportKusvLaneCsvFilePath);
	std::vector<std::string> vecStrLaneCsvFiles = FindFileList(strImportKusvLaneCsvFilePath, ".csv");
	...
	for( auto vecIt = vecStrLaneCsvFiles.begin(); vecIt != vecStrLaneCsvFiles.end(); vecIt++ )
	{
		// Lane file path
		std::string strLaneFile = strImportKusvLaneCsvFilePath + "/" + *vecIt + ".csv";
		...
		while (!ifsFile.eof()) {
			...
			// Parse
			tmpKusvLane.m_vecKusvLanePoint.push_back( SKusvLanePoint((double)stof(vecstrSplitLine[0]), (double)stof(vecstrSplitLine[1])) );
		}
...
bool SKusvLanes::ExportKusvLaneCsvFile(std::string strExportKusvLaneCsvFilePath)
{
	// Generate path
	...
	for( auto vecLanesIt = m_vecKusvLanes.begin(); vecLanesIt != m_vecKusvLanes.end(); vecLanesIt++ )
	{
		SKusvLane& tmpKusvLane = *vecLanesIt;
		
		// Configuration
		std::string strRecordingFile = strExportKusvLaneCsvFilePath + "/" + "KusvLane_" + std::to_string(tmpKusvLane.m_nLaneID) + ".csv";
		...
		for( auto vecLaneIt = tmpKusvLane.m_vecKusvLanePoint.begin(); vecLaneIt != tmpKusvLane.m_vecKusvLanePoint.end() ; vecLaneIt++ )
		{
			SKusvLanePoint& tmpKusvLanePoint = *vecLaneIt;
			
			file_out
				<< std::to_string(tmpKusvLanePoint.m_dPtX_m) << ","
...
std::vector<std::string> SKusvLanes::FindFileList(std::string strPath, std::string strExtension)
{
...
std::vector<std::string> SKusvLanes::SplitString(std::string strOrigin, std::string strTok)
{
	size_t cutAt;
...
void SKusvLanes::DisplayInfo(std::string str)
{
...
void SKusvLanes::DisplayProgress(std::string str, unsigned int nNumOfProgress, unsigned int nNumOfTotal)
{
	if (nNumOfTotal == 0) {
	...
	int pres_progress = (int)((double)nNumOfProgress / (double)nNumOfTotal * 100.);
